...
class TestRunner {
    ...
    private currentBrowserType: string | null;
    private screenshotsDir: string;

    constructor(screenshotsDir: string | null = null) {
        this.browser = null;
        this.context = null;
        this.page = null;
    ...
    async initializeBrowser(options: TestRunnerOptions = {}): Promise<void> {
        const {
            headless = false,
        ...
        try {
            ...
            logger.error('❌ Failed to launch browser', { browserType, error: err });
    ...
    async closeBrowser(): Promise<string | null> {
        try {
    ...
    async executeStep(step: StepConfig, executionId: string, options: TestRunnerOptions = {}): Promise<StepResult> {
        ...
        try {
            ...
            if (options.enableScreenshots) {
                ...
                try {
                    ...
                    const err = screenshotError instanceof Error ? screenshotError : new Error(String(screenshotError));
                    logger.error('Failed to take automatic screenshot:', { error: err });
                }
            }
            ...
            stepResult.logs.push(`Step failed: ${errorMessage}`);

            // Take screenshot on error
            try {
                stepResult.screenshot = await this.takeErrorScreenshot(executionId, step.stepId || 'unknown');
            } catch (screenshotError: unknown) {
                const err = screenshotError instanceof Error ? screenshotError : new Error(String(screenshotError));
                logger.error('Failed to take error screenshot:', { error: err });
            }
        }
    ...
    async executeNavigate(config: any): Promise<void> {
    ...
    async executeClick(config: any): Promise<void> {
    ...
    async executeType(config: any): Promise<void> {
    ...
    async executeWait(config: any): Promise<void> {
    ...
    async takeScreenshot(executionId: string, stepId: string, fullPage: boolean = false): Promise<string | null> {
    ...
    async executeVerify(config: any): Promise<void> {
        const rawSelector = config.selector || config.target || '';
    ...
    async executeDropdown(config: any): Promise<void> {
    ...
    async executeRefresh(config: any): Promise<void> {
    ...
    async takeErrorScreenshot(executionId: string, stepId: string): Promise<string | null> {
        try {
    ...
    async waitForElement(rawSelector: string, timeout: number = 5000): Promise<boolean> {
        try {
            const selector = this.normalizeSelector(rawSelector);
            if (this.page) {
